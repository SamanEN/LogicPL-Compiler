...
varDeclaration returns[VarDecStmt varDeclarationRet]:
    ...
    (ASSIGN e = expression {$varDeclarationRet.setInitialExpression($e.expressionRet);})?
    SEMICOLON
    ;
...
arrayDeclaration returns[ArrayDecStmt arrayDeclarationRet]:
    ...
    (a = arrayInitialValue {$arrayDeclarationRet.setInitialValues($a.arrayInitialValueRet);})?
    SEMICOLON
    ;
...
arrayList returns[ArrayList<Expression> arrayListRet]:
    ...
    LBRACKET
    ( v1 = value{$arrayListRet.add($v1.valueRet);} | i1 = identifier{$arrayListRet.add($i1.identifierRet);} )
    (COMMA ( v2 = value{$arrayListRet.add($v2.valueRet);} | i2 = identifier{$arrayListRet.add($i2.identifierRet);} ))*
    RBRACKET
    ;
...
returnSmt returns[ReturnStmt returnSmtRet]:
    RETURN
    (
        v = value {$returnSmtRet = new ReturnStmt($v.valueRet);}
        | i = identifier {$returnSmtRet = new ReturnStmt($i.identifierRet);}
    )?
    SEMICOLON
    {
...
andExpr2 returns[BinaryExpression andExpr2Ret] locals[BinaryExpression temp]:
...
eqExpr2 returns[BinaryExpression eqExpr2Ret] locals[BinaryExpression temp, BinaryOperator operator]:
    ( op = EQ {$operator = BinaryOperator.eq;} | op = NEQ {$operator = BinaryOperator.neq;} )
    left = compExpr right = eqExpr2
    {
        if($right.eqExpr2Ret != null)
        {
...
compExpr2 returns[BinaryExpression compExpr2Ret] locals[BinaryExpression temp, BinaryOperator operator]:
    ( op = LT {$operator = BinaryOperator.lt;} | op = LTE {$operator = BinaryOperator.lte;} | op = GT {$operator = BinaryOperator.gt;} | op = GTE {$operator = BinaryOperator.gte;} )
    left = additive right = compExpr2
    {
        if($right.compExpr2Ret != null)
        {
            ...
            $compExpr2Ret = new BinaryExpression(null, $temp, $operator);
        }
        else
        {
...
additive2 returns[BinaryExpression additive2Ret] locals[BinaryExpression temp, BinaryOperator operator]:
    ...
    left = multicative right = additive2
    {
        if($right.additive2Ret != null)
        {
            ...
            $additive2Ret = new BinaryExpression(null, $temp, $operator);
        }
        else
        {
...
multicative2 returns[BinaryExpression multicative2Ret] locals[BinaryExpression temp, BinaryOperator operator]:
    ( op = MULT {$operator = BinaryOperator.mult;} | op = DIV {$operator = BinaryOperator.div;} | op = MOD {$operator = BinaryOperator.mod;} )
    left = unary right = multicative2
    {
        if($right.multicative2Ret != null)
        {
            ...
            $multicative2Ret = new BinaryExpression(null, $temp, $operator);
        }
        else
        {
...
unary returns[Expression unaryRet] locals[UnaryOperator operator]:
    ...
    |
    ( op = PLUS {$operator = UnaryOperator.plus;} | op = MINUS {$operator = UnaryOperator.minus;} | op = NOT {$operator = UnaryOperator.not;} )
    o = other
    {
        $unaryRet = new UnaryExpression($operator, $o.otherRet);
        $unaryRet.setLine($op.getLine());
    }
    ;
